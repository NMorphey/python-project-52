...
class UnavailableForGuestsTestCase(SetUpUsers):
    ...
    def test_category_on_main_page(self):
        ...
        self.assertNotContains(response, reverse_lazy('labels_index'))

        self.client.post(reverse_lazy('login'), self.user_1_login_data)
        response = self.client.get(reverse_lazy('main_page'))
        self.assertContains(response, reverse_lazy('labels_index'))
...
class CRUDTestCase(SetUpLabel):
    ...
    def test_delete(self):
        self.client.post(reverse_lazy('delete_label', kwargs={'pk': 1}))
        response = self.client.get(reverse_lazy('labels_index'))
